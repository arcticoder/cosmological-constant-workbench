...
\title{\Large\bfseries Constraints on Loop Quantum Gravity Predictions for the Cosmological Constant\\
...
\textbf{Index Terms}---Cosmological constant problem, loop quantum gravity, vacuum energy, parameter constraints, phenomenology.

\section{Introduction}
The cosmological-constant problem is often framed as a hierarchy between naive vacuum-energy scales and the observed late-time dark-energy density~\cite{weinberg1989,carroll2001}.
...
Loop quantum gravity (LQG) approaches to the cosmological constant have proposed that polymer quantization and spin-foam amplitudes might provide natural suppression mechanisms~\cite{rovelli2004,thiemann2007}.
...
All code, data, and build scripts are available at \url{https://github.com/DawsonInstitute/cosmological-constant-workbench}~\cite{ccw_repo}.

\section{Workbench and Target Integration}
We use a modular Python ``workbench''~\cite{ccw_repo} that:
...
(iii) provides constrained ``phenomenological'' mechanisms (e.g., holographic dark energy~\cite{li2004}, sequestering~\cite{kaloper2014}) which can be tuned to a target $\rho_{\Lambda}$.
...
It does not rule out all LQG approaches to $\Lambda$~\cite{rovelli2004}.
...
or (iii) additional dynamics (e.g., true sequestering-like cancellations~\cite{kaloper2014}) not present in the predictor.
...
This work was supported by internal research and development funds. The authors thank the quantum gravity community for helpful discussions.

\bibliographystyle{ieeetr}
\bibliography{lqg_cc_constraints}
